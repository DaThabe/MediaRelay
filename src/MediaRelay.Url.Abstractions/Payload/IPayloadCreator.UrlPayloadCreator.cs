    ...
    where TUrlContent : IUrlContent
{
    ...
    /// </summary>
    public virtual bool CanCreate(IContent content) => content is TUrlContent;

    /// <summary>
    /// 创建转发数据，并存储内容中的所有资源
    /// </summary>
    /// <exception cref="NotSupportedException">不支持的网址内容类型</exception>
    public virtual async ValueTask<IPayload> CreateAsync(IContent content, CancellationToken cancellationToken = default)
    {
